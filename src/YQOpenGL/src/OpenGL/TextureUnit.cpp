#include "..\..\include\OpenGL\TextureUnit.h"
...
namespace YQ::OpenGL
{
	TextureUnit::TextureUnit(TextureType type, const std::string_view objectName)
	{
		glGenTextures(1, &m_nTextureID);
		glBindTexture(GL_TEXTURE_2D, m_nTextureID);

		m_strObjName = objectName;
	}
	...
	bool TextureUnit::Build(uint32_t nGlTexture)
	{
		if (nGlTexture != 0)
		{
	...
	uint32_t TextureUnit::TextureNum()
	{
		return m_nGlTextureNum - GL_TEXTURE0;
	}

	void TextureUnit::SetParameteri(uint32_t type, uint32_t around)
	{
		if (m_gTextureID != m_nTextureID)
		{
			m_gTextureID = m_nTextureID;
	...
	uint32_t TextureUnit::m_gTextureID = 0;
}